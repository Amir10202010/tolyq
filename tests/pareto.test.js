...
suite('pareto.js — фронт на реальной сети', () => {
  test('прямое плечо Астана→Караганда: фронт содержит РОВНО две ожидаемые точки', () => {
    // Простейшая нетривиальная задача: два смежных узла, два вида
    // транспорта. Правильный ответ известен заранее и состоит ровно
    // из двух прямых маршрутов — фура быстрее, вагон дешевле и чище.
    // Любой обходной путь через Павлодар хуже по всем трём критериям
    // сразу и обязан быть вытеснен, а не остаться во фронте.
    const { pareto, dominated } = paretoRoutes('AST', 'KGF', req({ to: 'KGF' }));

    equal(pareto.length, 2, `во фронте ${pareto.length} точек вместо двух: ${pareto.map((r) => r.label).join(', ')}`);
    ...
    // Оба — прямые, в один участок, без перегрузок
    equal(road.legs.length, 1, 'автомобильный вариант должен быть прямым');
    equal(rail.legs.length, 1, 'железнодорожный вариант должен быть прямым');
    equal(road.transshipments, 0);
    equal(rail.transshipments, 0);

    // Компромисс идёт ровно по тем осям, по которым и должен
    ...
    // Обходные пути найдены и честно отброшены, а не пропущены
    greaterOrEqual(dominated.length, 1, 'перебор обязан был найти и отбросить обходные пути');
    for (const d of dominated) {
      assert(
        d.legs.length > 1 || d.hours > Math.min(road.hours, rail.hours),
        `во отброшенных оказался прямой лучший маршрут: ${d.label}`
      );
    }
  });
